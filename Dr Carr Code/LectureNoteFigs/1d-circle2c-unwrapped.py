...
ax.plot(np.pi/4,0,'bo',markersize=15)
ax.plot(3*np.pi/4,0,'bo',fillstyle='none',markersize=15)
...
ax.arrow(3*np.pi/4, 0, -np.pi/4, 0.0, head_width=0.1, head_length=0.2, fc='k', ec='k')
ax.arrow(3*np.pi/4, 0, np.pi/4, 0.0, head_width=0.1, head_length=0.2, fc='k', ec='k')

ax.set_title(r"$\dot{\theta}=\omega-a\sin\theta, a>\omega$",fontsize=16)

fig.savefig("1d-circle2c-unwrapped.pdf")
plt.show()
